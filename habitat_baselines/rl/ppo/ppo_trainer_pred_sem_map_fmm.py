...
from habitat_baselines.common.semexp.model import RL_Policy as SemExpRLPolicy, Semantic_Mapping
...
from habitat_baselines.common.semantic_segmentation.semexp_maskrcnn.semantic_prediction import SemanticPredMaskRCNN
from habitat_baselines.common.semantic_segmentation.semexp_maskrcnn import arguments
import habitat_baselines.common.semexp.pose as pu
...
@baseline_registry.register_trainer(name="semexpfmm")
class PredSemSemExpFmmTrainer(BaseRLTrainer):
    r"""Trainer class for predicted semantic map similar to the modular approach in Home Robot
    ...
    def _eval_checkpoint(
        ...
        self.pred_acc_bg_obj = [[] for _ in range(self.config.NUM_ENVIRONMENTS)]
        self.pred_acc_obj = [[] for _ in range(self.config.NUM_ENVIRONMENTS)]
        self.pred_precision_obj = [[] for _ in range(self.config.NUM_ENVIRONMENTS)]
        self.pred_recall_obj = [[] for _ in range(self.config.NUM_ENVIRONMENTS)]
        ...
        # Mask-RCNN from Home-Robot and SemExp
        # initialize semantic segmentation prediction model
        args = self.config.RL.POLICY.SemExp_MaskRcnn
        self.sem_pred = SemanticPredMaskRCNN(args.sem_pred_prob_thr)
        #
        
        # Semantic Mapping
        args = self.config.RL.POLICY.SemExp_mapping
        self.sem_map_module = Semantic_Mapping(args, self.device).to(self.device)
        self.sem_map_module.eval()
        
        self.poses = torch.zeros(self.config.NUM_ENVIRONMENTS, 3, dtype=torch.float, device=self.device)
        self.last_poses = torch.zeros(self.config.NUM_ENVIRONMENTS, 3, dtype=torch.float, device=self.device)
        self.local_pose = torch.zeros(self.config.NUM_ENVIRONMENTS, 3, dtype=torch.float, device=self.device)
        self.nc = self.config.RL.POLICY.semexp_num_sem_categories + 4
        self.full_w, self.full_h = self.map_grid_size, self.map_grid_size
        self.local_w = int(self.full_w / args.global_downscaling)
        self.local_h = int(self.full_h / args.global_downscaling)
        self.local_map = torch.zeros(self.config.NUM_ENVIRONMENTS, self.nc, self.local_w, self.local_h, dtype=torch.float, device=self.device)
        
        # initialize local pose
        for ind in range(self.config.NUM_ENVIRONMENTS):
            self.local_pose[ind] = torch.tensor([self.local_w//2, self.local_h//2, 0], dtype=torch.float, device=self.device)
        
        # self.full_pose = torch.zeros(self.config.NUM_ENVIRONMENTS, 3, dtype=torch.float, self.device)
        # self.full_pose[:, :2] = self.map_grid_size / 100.0 / 2.0
                ...
                if prev_actions[i].item() == 0:
                    ...
                    if len(goal_grid_loc) == 0:
                        ...
                        # SemExp Global policy (exploration)
                            ...
                            sem_id = [maps.OBJNAV_CATEGORY_v2_TO_SEMEXP_15[s.item()] if s.item() in maps.OBJNAV_CATEGORY_v2_TO_SEMEXP_15 else 0 for s in sem_id]
                            if len(sem_id) > 0:
                            ...
                            if goal_cat in maps.OBJNAV_CATEGORY_v2_TO_SEMEXP_15:
                                extras[0, 1] = maps.OBJNAV_CATEGORY_v2_TO_SEMEXP_15[goal_cat] # append goal category
                ...
                # episode continues
                    ...
                    if len(self.config.VIDEO_OPTION) > 0:
                        ...
                        if self.config.VIDEO_RENDER_ALL_INFO:
                            ...
                            # _m["prediction_recall"] = (np.nanmean(np.array(self.pred_recall_obj[i])) if len(self.pred_recall_obj[i])>0 else 0)
                            # _m["prediction_f1_score"] = (np.nanmean(np.array(self.pred_f1_score[i])) if len(self.pred_f1_score[i])>0 else 0)
                            _m["predicted_objs"] = str(np.unique(semantic[i]))
                            _m["predicted_obj_names"] = str(np.array([maps.OBJNAV_CATEGORY_MAP[ind] for ind in np.unique(semantic[i])]))
                            # _m["GT_objs"] = str(np.unique(self.gt_semantic[i]))
                            # _m["GT_objs_names"] = str(np.array([maps.OBJNAV_CATEGORY_MAP[ind] for ind in np.unique(self.gt_semantic[i])]))
                            frame = overlay_frame(frame, _m)
    ...
    def build_map(self, observations, object_maps, grid_map):
        ...
        theta = observations['episodic_compass']
        location = observations["episodic_gps"]
        curr_pose = torch.concat((location, theta), axis=-1)
        for ind, p in enumerate(self.poses):
            dx, dy, do = pu.get_rel_pose_change(curr_pose[ind].cpu().numpy(), p.cpu().numpy())
            self.poses[ind] = torch.tensor([dx, dy, do], dtype=torch.float32)
            self.last_poses[ind] = curr_pose[ind]
        
        rgb = observations["rgb"] #.type(torch.float32)
        semantic = np.zeros([rgb.shape[0], rgb.shape[1], rgb.shape[2]])
        sem_map = torch.zeros(rgb.shape[0], rgb.shape[1], rgb.shape[2], 16)

        for i, im in enumerate(rgb):
            img = im.cpu().numpy()
            p = self.sem_pred.predict(img)
            
            # instances = p["instance_classes"]
            # Image.fromarray(p['semantic_frame']).save(f"debug/out{i}.jpg")
            
            cls_ids = np.nonzero(p['semantic']>0)
            
            if len(cls_ids[0]) > 0:
                semantic[i,:,:] = p['semantic']
                # semantic[i,:,:,1] = p['score_map'] * 100
        
        _indices = semantic.nonzero()
        if len(_indices[0]) > 0:
            sem_id = semantic[_indices[0], _indices[1], _indices[2]]
            if len(sem_id) > 0:
                sem_map[_indices[0], _indices[1], _indices[2], sem_id] = 1
        sem_map = sem_map.permute(0,3,1,2)
        
        _, self.local_map, _, self.local_pose = self.sem_map_module(observations, sem_map, self.poses, self.local_map, self.local_pose)
        
        _map = self.local_map.permute(0, 2, 3, 1)
        sem_label_map = torch.argmax(_map[:, :, :, 4:], axis=-1)
        object_maps[:, :, :, 1] = sem_label_map
        object_maps[:, :, :, 0] = _map[:, :, :, 0]  # Occupancy
        # object_maps[:, :, :, 0] = _map[:, :, :, 0]  # Explored
        
        _agent_locs = self.to_grid(location.cpu().numpy())
    ...
    def to_grid_t(self, xy):
        return (torch.round(xy / self.map_resolution) + torch.from_numpy(self.map_center).to(self.device)).type(torch.long)
        
    def from_grid_t(self, grid_x, grid_y):
        return [
            (grid_x - self.map_center[0]) * self.map_resolution,
            (grid_y - self.map_center[1]) * self.map_resolution,
            ]
