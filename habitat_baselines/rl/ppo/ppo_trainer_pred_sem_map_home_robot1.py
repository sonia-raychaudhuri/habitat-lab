...
@baseline_registry.register_trainer(name="homerobotvar")
class PredSemHomeRobotTrainer(BaseRLTrainer):
    ...
    def _eval_checkpoint(
        ...
        self.sem_pred = SemanticPredMaskRCNN(sem_pred_prob_thr=args.sem_pred_prob_thr)
        #
    ...
    def build_map(self, observations, object_maps, grid_map):
        ...
        for i, im in enumerate(rgb):
            ...
            obs = self.sem_pred.predict(img)
            
            img = im.cpu().numpy().astype(np.uint8)
            semantic_pred, rgb_vis = self.sem_pred.get_prediction(img)
            semantic_pred = np.argmax(semantic_pred, axis=-1).astype(np.uint8)
            
            _indices = semantic_pred.nonzero()
            if len(_indices[0]) > 0:
                semantic_pred[_indices[0], _indices[1]] = [maps.OBJNAV_CATEGORY_v2_TO_SEMEXP_15_INDEX_MAP[s] for s in semantic_pred[_indices[0], _indices[1]]]
            
            # ind = time.time()
            # Image.fromarray(rgb_vis.astype(np.uint8)).save(f"debug/pred_mask_{ind}.png")
