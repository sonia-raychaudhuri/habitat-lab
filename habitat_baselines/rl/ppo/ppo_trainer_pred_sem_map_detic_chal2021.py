...
@baseline_registry.register_trainer(name="preddetic21")
class PredSemMapDeticChal2021OnTrainer(BaseRLTrainer):
                ...
                if not not_done_masks[i].item():
                    ...
                    if len(self.config.VIDEO_OPTION) > 0:
                        ...
                        if self.config.VIDEO_RENDER_ALL_INFO:
                            ...
                            _m["next_goal"] = maps.OBJNAV_21_CATEGORY_MAP[next_goal_category[i].item()+1]
                            if self.config.RL.POLICY.EXPLORATION_STRATEGY == "stubborn":
                            ...
                            _m["predicted_obj_names"] = str(np.array([maps.OBJNAV_21_CATEGORY_MAP[ind] for ind in np.unique(semantic[i,:,:,0])]))
                            _m["GT_objs"] = str(np.unique(self.gt_semantic[i]))
                            _m["GT_objs_names"] = str(np.array([maps.OBJNAV_21_CATEGORY_MAP[ind] for ind in np.unique(self.gt_semantic[i])]))
