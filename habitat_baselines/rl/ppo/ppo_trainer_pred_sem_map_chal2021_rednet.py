...
@baseline_registry.register_trainer(name="predrednetchal2021")
class PredSemMapRedNetChal2021OnTrainer(BaseRLTrainer):
    ...
    def build_map(self, observations, object_maps, grid_map):
        ...
        rgb = (rgb / 255.0).permute(0,3,1,2)
        # rgb = torch.FloatTensor(rgb).permute(0,3,1,2)
        rgb = self.normalize(rgb)
        # rgb = rgb.unsqueeze(0).to(device)
        
        depth_enc = observations["depth"][:,:,:,0]
        depth_enc = depth_enc.type(torch.float32)
        # depth_enc = torch.FloatTensor(depth_enc).unsqueeze(0)
        depth_enc = self.depth_normalize(depth_enc)
        depth_enc = depth_enc.unsqueeze(1)
        # depth_enc = depth_enc.unsqueeze(0).to(device)
        
        output, _ = self.model_rednet(rgb, depth_enc)
        semantic = torch.max(output, 1)[1]
        semantic = semantic.unsqueeze(-1).cpu().numpy()
        # if len(np.unique(semantic)) > 1:
        #     print('Unique values: ') 
        
        semantic = maps.OBJNAV_CATEGORY_12_TO_21[semantic]
