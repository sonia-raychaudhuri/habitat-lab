...
def transform_pose(XYZ, location, theta):
    ...
    """
    sh = XYZ.shape
    R = ru.get_r_matrix([0., 0., 1.], angle=theta - np.pi / 2.)
    XYZ = np.matmul(XYZ.reshape(sh[0], sh[1]*sh[2], 3), np.transpose(R, axes=(0,2,1))).reshape(XYZ.shape)
    XYZ[:, :, :, 0] = XYZ[:, :, :, 0] + location[:, 0][..., np.newaxis, np.newaxis]
    XYZ[:, :, :, 1] = XYZ[:, :, :, 1] + location[:, 1][..., np.newaxis, np.newaxis]
    return XYZ
...
def bin_points_w_sem_w_score(XYZS_cms, map_size, z_bins, xy_resolution, map_center):
    """Bins points into xy-z bins
    XYZS_cms is ... x H x W x4 -> 3D coordinates + semantic channel
    Outputs is ... x map_size x map_size x (len(z_bins)+1)
    """
    ...
    Z_bin = np.digitize(XYZS_cms[:, :, :, 2], bins=z_bins).astype(np.int32)
    S_bin = XYZS_cms[:, :, :, 3].astype(np.int32)
    S_score_bin = XYZS_cms[:, :, :, 4].astype(np.int32)
    ...
    for i in range(sh[0]):
        ...
        sem_score = count.copy()
        sem_score[ind[i][isvalid[i]]] = S_score_bin[i][isvalid[i]]    #  replace count with the semantic label
    ...
    return counts, sem_labels, sem_scores

def project_sem(XYZS_cms, map_size, z_bins, xy_resolution, map_center):
    """Bins points into xy-z bins
    XYZS_cms is ... x H x W x4 -> 3D coordinates + semantic channel
    Outputs is ... x map_size x map_size x (len(z_bins)+1)
    """
...
def project_sem_w_scores_0(XYZS_cms, map_size, z_bins, xy_resolution, map_center):
    """Bins points into xy-z bins
    XYZS_cms is ... x H x W x4 -> 3D coordinates + semantic channel
    Outputs is ... x map_size x map_size x (len(z_bins)+1)
    """
    ...
    for i in range(sh[0]):
        ...
        sem_score = np.zeros((map_size * map_size))
        sem_score[ind[i]]=S_score_bin[i]
        sem_scores.append(np.reshape(sem_score, [map_size, map_size]))
    
    sem_labels = np.concatenate(sem_labels, axis=0).reshape([sh[0], map_size, map_size])
    sem_scores = np.concatenate(sem_scores, axis=0).reshape([sh[0], map_size, map_size])

    return sem_labels, sem_scores
