...
from habitat_baselines.common import map_utils as multion_maps
...
def observations_to_image(observation: Dict, projected_features: np.ndarray=None, 
        ...
        config: np.ndarray=None, agent_loc: np.ndarray=None, agent_angle=None) -> np.ndarray:
    ...
    if "semantic" in observation:
        ...
        semantic_map = maps.colorize_topdown_map(semantic_map)
        egocentric_view.append(semantic_map)
        
    if "semantic_labels" in observation:
        observation_size = observation["semantic_labels"].shape[0]
        semantic_map = observation["semantic_labels"].squeeze()
    ...
    if object_map is not None:
        ...
        # occupancy
        occupancy = (np.maximum(np.maximum(object_map[:,:,0],object_map[:,:,2]),object_map[:,:,3])).astype(np.uint8) #object_map[:,:,0].astype(np.uint8)
        #occ_map = multion_maps.OCC_MAP_COLORS[occ_map]
        occ_map = multion_maps.OBJECT_MAP_COLORS[occupancy]
        ...
        # with FOW
        # occ_map = multion_maps.OBJECT_MAP_COLORS[occupancy]
        # _fog_of_war_mask = np.zeros_like(occupancy)
        ...
        # fog_of_war_desat_values = np.array([[0.5], [1.0]])
        # # Only desaturate things that are valid points as only valid points get revealed
        # desat_mask = occupancy != 0

        # occ_map[desat_mask] = (
        #     occ_map * fog_of_war_desat_values[_fog_of_war_mask]
    ...
    if 'object_map' in observation:
        gt_maps = observation['object_map'][:,:,0]
        if not isinstance(gt_maps, np.ndarray):
            gt_maps = gt_maps.cpu().numpy()
            
        gt_maps = gt_maps.astype(np.uint8)
        gt_maps = multion_maps.OBJECT_MAP_COLORS[gt_maps]
        ...
        frame = np.concatenate((frame, gt_maps), axis=1)
