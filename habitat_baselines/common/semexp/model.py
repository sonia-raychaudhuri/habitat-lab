...
from habitat_baselines.common.semexp.utils.distributions import Categorical, DiagGaussian
from habitat_baselines.common.semexp.utils.model import get_grid, ChannelPool, Flatten, NNBase
import habitat_baselines.common.semexp.envs.utils.depth_utils as du
...
class Semantic_Mapping(nn.Module):
    ...
    def forward(self, obs, semantic_map, pose_obs, maps_last, poses_last):
        
        depth = (obs['depth'] * 10).squeeze(-1)
        # depth[depth == 0] = np.NaN

        bs, h, w = depth.size()
        c = semantic_map.shape[1]
        ...
        self.feat[:, 1:, :] = nn.AvgPool2d(self.du_scale)(
             semantic_map
        ).view(bs, c, h // self.du_scale * w // self.du_scale)
